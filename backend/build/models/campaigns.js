  ...
  db.campaignContent.update({ contentStatus: 'previous' }, {
    ...
    db.campaignContent.create({
      ...
      db.campaignText.bulkCreate(blocks.text).then(() => {
        db.campaignImages.bulkCreate(blocks.images).then(() => {
          success({
            statusCode: 200,
...
const getCampaigns = exports.getCampaigns = ({ page, search, sort }, success, error) => {
  db.campaigns.findAll({
    where: {
      startDate: {
        $ne: null
    ...
    const allCampaigns = findCampaignsResults.reduce((cleanedCampaigns, campaign) => {
      const {
        campaignId,
        name,
        duration,
      ...
      const campaignParagraphs = campaignContents[0].campaignTexts.reduce((paragraphs, block) => {
      ...
      const firstParagraph = campaignParagraphs[0].nodes.reduce((text, node) => {
        if (node.type === 'link') {
      ...
      const campaignMainImgs = campaignContents[0].campaignImages.reduce((images, img) => {
    ...
    const sortCampaigns = () => {
      switch (sort) {
        case 'Percent Funded':
          return searchedCampaigns.sort((a, b) => b.donationsMade / b.fundingNeeded - a.donationsMade / a.fundingNeeded);
        case 'Days Remaining':
          return searchedCampaigns.sort((a, b) => Date.parse(a.endDate) - Date.parse(b.endDate));
        case 'Funding Needed':
